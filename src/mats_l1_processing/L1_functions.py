...
# This function is now in L1_calibration functions - link to that file instead. 
...
#     #bad column analysis #LM201025 nread is the number of bits one super column has been devidet into before it is read. ,Coadd is numer of total columns in a supersupercolumn (FPGA and onchip binned)
#     n_read, n_coadd = binning_bc(int(header['NCol'])+1, int(header['NColSkip']), 2**int(header['NColBinFPGA']), ncolbinC, header['BadCol'])
    
#     #go through the columns
#     for j_c in range(0, int(header['NCol'])+1):#LM201102 Big fix +1 added and simplified code below
...
#         true_image[:, j_c] = true_image[:, j_c] - n_read[j_c]*(header['BlankTrailingValue']-128)-128
...
#         true_image[:, j_c] = true_image[:, j_c] * (2**int(header['NColBinFPGA'])*ncolbinC/n_coadd[j_c])
        
#         #print('Print1919 Col: ',j_c,', n_read: ',n_read[j_c],', n_coadd: ',n_coadd[j_c],', binned pixels: ',ncolbinC*2**int(header['NColBinFPGA']))
    
        
#     return true_image

# This fiunction is now in L1_calibration_functions. Link to htat istead. LM201215
...
# This function is now in L1_calibration fucntions- link to taht instead.
...
#             image[irow,:]=image[irow,:] - image[krow,:]*(T_row_extra/T_exposure)
...
# # This function is now in L1_calibration_functions- link to that instead. LM201215
...
# # This function is now in L1_calibration_functions- link to that instead. LM201217
