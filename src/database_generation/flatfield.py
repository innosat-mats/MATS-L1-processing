...
def make_flatfield(channel, signalmode, calibration_file,plot=True):
    ...
    diff_field_smooth=ndimage.gaussian_filter(diff_field, sigma=4) #sigma = 4 pixels for gaussan smoothing kernel?
    ...
    if plot:
        ...
        plot_CCDimage(flatfield_morphed_minus_wo_scaled[ypmin:ypmax,xpmin:xpmax],fig,ax[7], title='flatfield_morphed-flatfield_w_baffle_scaled')
