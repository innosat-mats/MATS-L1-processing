...
def compensate_flatfield(CCDitem, image='No picture'):
    ...
    mean=image_flatf_fact[CCDitem['NRSKIP']:CCDitem['NRSKIP']+CCDitem['NROW'],
                                      CCDitem['NCSKIP']:CCDitem['NCSKIP']+CCDitem['NCOL']+1].mean()
    shape=image_flatf_fact[CCDitem['NRSKIP']:CCDitem['NRSKIP']+CCDitem['NROW'],
                                      CCDitem['NCSKIP']:CCDitem['NCSKIP']+CCDitem['NCOL']+1].shape

    image_flatf_comp=image/image_flatf_fact[CCDitem['NRSKIP']:CCDitem['NRSKIP']+CCDitem['NROW'],
...
def calculate_flatfield(CCDitem):
    ...
    if CCDitem['NCBIN CCDColumns']>1 or CCDitem['NCBIN FPGAColumns']>1: #Or row binning
...
def subtract_dark(CCDitem, image='No picture'):
    ...
    image_dark_sub=image-dark_fullpic[CCDitem['NRSKIP']:CCDitem['NRSKIP']+CCDitem['NROW'],
                                      CCDitem['NCSKIP']:CCDitem['NCSKIP']+CCDitem['NCOL']+1]
                                #rows,colums Note that nrow always seems to be implemented as +1 already, whereas NCOL does not, hence the missing '+1' in the column calculation /LM201204
    return image_dark_sub
...
#    CCDunit=CCD(CCDitem['channel'])   
    ...
    if CCDitem['NCBIN CCDColumns']>1 or CCDitem['NCBIN FPGAColumns']>1: #Or row binning
        dark_calc_image=bin_image_using_predict_and_get_true_image(CCDitem, dark_calc_image)
...
#                        print('start bin mycolumn=',(j_c)*ncolbinC*ncolbinF + j_bc + ncolskip)
                    ...
                    if ncolbinC > 1 and (j_c)*ncolbinC*ncolbinF + j_bc + ncolskip in (bad_columns):  #KTH:should be here +1 becuase Ncol is +1
                        ...
                        + reference_image[(j_r)*nrowbin+j_br+nrowskip,(j_c)*ncolbinC*ncolbinF+j_bc+ncolskip] #row and column value evaluation
                        * 1 #scaling factor
                        )
...
def bin_image_using_predict(header, reference_image='999'):
    ...
    #gain=2**(int(header['Gain']) & 255)
    gain=2.**0 # TODO htis should be read in! LM201211
...
def get_true_image(header, image='No picture'):
    ...
    #remove gain
    true_image = image * 2**(int(header['DigGain']))  #TODO I dont think htis should be DigGain . Says Gain in original coding.  Check with Nickolay LM 201215 
    ...
    for j_c in range(0, int(header['NCOL']+1)):#LM201102 Big fix +1
        ...
        true_image[0:int(header['NROW'])+1, j_c] = true_image[0:int(header['NROW']+1), j_c] - n_read[j_c]*(header['TBLNK']-128)-128
        
        #compensate for bad columns
        true_image[0:int(header['NROW'])+1, j_c] = true_image[0:int(header['NROW']+1), j_c] * (2**int(header['NColBinFPGA'])*ncolbinC/n_coadd[j_c])
...
def get_true_image_reverse(header, true_image='No picture'):
    ...
    #add gain
    image = true_image / 2**(int(header['DigGain'])) #TODO I dont think htis should be DigGain . Says Gain in original coding.  Check with Nickolay LM 201215 
...
def desmear_true_image(header, image='No picture'):
    ...
    T_exposure = float(header['TEXPMS'])/1000.
...
def compensate_bad_columns(header,  image='No picture'):
    ...
    gain=2**(int(header['DigGain'])) #TODO I dont think htis should be DigGain . Says Gain in original coding.  Check with Nickolay LM 201215 
...
def read_flatfield(CCDunit, mode):
    ...
    if CCDunit.channel=='NADIR': #Hack since we dont have any nadir flat fields yet. 
        ...
        try: flatfield=CCDItemsUnitsSelect[0].subpic  # This is where it gets read in. The dark (including offsets and balnks) have already been subracted. 
...
def readimg(filename):
    ...
    if len(data_arr)< NRow*(NCol+1)/(2**(NColBinFPGA)):#check for differences in python 2 and 3
        ...
        img_flag = 1
        image = np.reshape(np.double(data_arr[11+1:NRow*(NCol+1)+12]), (NRow,NCol+1)) #LM201102 Corrected bug where4 NCol and NRow were switched
...
def readimgpath(path, file_number, plot):
    ...
    if plot>0:
        ...
        plt.imshow(image, cmap='jet', vmin=mean_img-100, vmax=mean_img+100,  aspect='auto')
        plt.title('CCD image')
        plt.xlabel('Pixels')
        plt.ylabel('Pixels')
